...
static void app_send_failed(DictionaryIterator* failed, AppMessageResult reason, void* context) {
	...
	http_callbacks.failure(1, 1000 + reason, context);
	
}

static void app_dropped(void* context, AppMessageResult reason) {
	if(!http_callbacks.failure) return;
	http_callbacks.failure(2, 1000 + reason, context);
	//http_log("App dropped");
}
...
HTTPResult http_log(const char* value) {
	...
	DictionaryResult dict_result = dict_write_cstring(iter, HTTP_LOG_KEY, value);
	if(dict_result != DICT_OK) {
		return dict_result << 12;
	}
	app_result = app_message_out_send();
...
HTTPResult http_battery_request() {
	...
	DictionaryResult dict_result = dict_write_uint8(iter, HTTP_BATTERY_KEY, 1);
	if(dict_result != DICT_OK) {
		return dict_result << 12;
	}
	app_result = app_message_out_send();
